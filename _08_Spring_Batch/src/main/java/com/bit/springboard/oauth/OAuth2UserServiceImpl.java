...
public class OAuth2UserServiceImpl extends DefaultOAuth2UserService {
    ...
    /*
       소셜 로그인 버튼 클릭 -> 인증 서버로 요청 -> 인증 서버에서 인증 코드 발급
       -> 발급받은 인증코드로 다시 한번 인증 서버로 요청 -> 인증 서버는 인증 코드의 유효성을 검사 후 토큰 발급
       -> 발급된 토큰으로 자원 서버에 요청 -> 자원 서버는 토큰의 유효성을 검사 한 후 사용자의 정보를 리턴
