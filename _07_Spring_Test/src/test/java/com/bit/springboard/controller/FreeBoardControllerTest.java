...
public class FreeBoardControllerTest {
    ...
    void findByIdTest() throws Exception {
        // 테스트 계획 수립
        given(apiService.findFreeBoardById(3L))
                .willReturn(
                        FreeBoard.builder()
                                .id(3L)
                                .title("ads")
                                .content("adssa")
                                .cnt(0)
                                .regdate(LocalDateTime.parse("2024-08-27T09:49:43.777287"))
                                .moddate(LocalDateTime.parse("2024-08-27T09:58:05.937422"))
        ...
        mockMvc.perform(get("/apis/boards/" + 3))
                //andExpect(): 결과값을 검증
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(3L))
                .andExpect(jsonPath("$.data.title").value("ads"))
                .andExpect(jsonPath("$.data.content").value("adssa"))
